...
class TransformersAna {
    constructor() {
        ...
        // Configuration
        this.modelName = 'microsoft/trocr-small-stage1'; // Best for printed documents
        this.quantized = true; // Use quantized model for smaller size
    }

    async init() {
        if (this.isInitializedT) return;
        
        console.log('🔄 Initializing Transformers.js OCR...');
    ...
    async initializeOCR() {
        ...
        if (this.isInitializedT && this.ocrModel) return;

        this.initializationPromise = (async () => {
            try {
                console.log('Loading Transformers.js OCR model...');
                
                this.ocrModel = await this.pipeline(
                    'image-to-text',
                    this.modelName,
                    {
                        quantized: this.quantized,
                        progress_callback: progress => this.updateOCRProgress(progress),
                    }
                );

                this.isInitializedT = true;
                console.log('✅ Transformers.js OCR initialized successfully');
    ...
    async debugInitialize() {
        try {
            console.log('🧪 Debug: Testing Transformers.js initialization...');
            
            const testModel = await this.pipeline(
                'image-to-text',
    ...
    updateOCRProgress(progress) {
        ...
        switch (progress.status) {
            case 'loading':
                loadingEl.textContent = 'Loading OCR model...';
                break;
            case 'ready':
                loadingEl.textContent = 'OCR model loaded';
                break;
            case 'running':
    ...
    async performOCR(imageFile) {
        if (!this.isInitializedT || !this.ocrModel) {
        ...
        try {
            ...
            // Run OCR
            const result = await this.ocrModel(processedImage);
            
            console.log(`✅ OCR completed. Text length: ${result[0].generated_text.length}`);
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    img.onload = async () => {
                        ...
                        // Optimize for OCR - resize and enhance
                        ...
                        // Enhance for text extraction (especially for Amharic)
    ...
    enhanceImageForOCR(imageData) {
        // Enhance contrast for better text extraction, especially for Amharic
        const data = imageData.data;
        const contrast = 1.3; // Increase contrast
        const brightness = -10; // Slightly darken
    ...
    getWorkerStatus() {
        return {
            ...
            model: this.ocrModel ? 'Loaded' : 'None',
    ...
    // Only OCR extraction changed, all text processing remains identical
    ...
    generateSummary(docType, text, title, topics, extractedData = {}, language) {
        ...
        return `
            <div class="document-summary">
                ...
                ${vehicleInfo}
                <p>Processed with Transformers.js (${this.modelName})</p>
            </div>
        `;
    }
    ...
    async destroy() {
        // Transformers.js doesn't require explicit termination
        this.ocrModel = null;
        this.isInitializedT = false;
    }
}

// Remove TesseractLanguageManager since Transformers.js doesn't need it
// Models are automatically downloaded and cached by Transformers.js

export const transformersAna = new TransformersAna();
export default transformersAna;